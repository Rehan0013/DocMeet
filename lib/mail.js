...
export const sendAppointmentCompletionEmail = async (appointment, doctor, patient) => {
  const patientHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #eee; border-radius: 10px;">
      ...
      <div style="text-align: center; margin: 30px 0;">
        <p style="margin-bottom: 15px;">How was your experience? Your feedback helps thousands of others find the right care.</p>
        <a href="${process.env.NEXT_PUBLIC_APP_URL}/reviews/new?appointmentId=${appointment.id}" 
      ...
      <a href="${process.env.NEXT_PUBLIC_APP_URL}/appointments" style="display: inline-block; padding: 10px 20px; background-color: #2D3FE2; color: white; text-decoration: none; border-radius: 5px;">View Appointment History</a>
  ...
  const adminHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #eee; border-radius: 10px;">
      <h2 style="color: #2D3FE2;">Appointment Completed</h2>
      <p>Appointment ID: ${appointment.id}</p>
      <p>Doctor: ${doctor.name}</p>
      <p>Patient: ${patient.name}</p>
      <p>Time: ${new Date(appointment.startTime).toLocaleString()}</p>
  ...
  await sendEmail({
    to: patient.email,
    subject: "Your Consultation is Complete - Leave a Review - DocMeet",
    html: patientHtml,
  });
